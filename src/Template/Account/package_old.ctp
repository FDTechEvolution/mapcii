...
<div class="container-fluid">
    <div id="package-payment-list" class="row">
            ...
            <table class="table g-mb-20" style="border-bottom: 1px solid #ddd;">
                <thead>
                    <tr class="g-bg-primary g-color-white">
                        ...
                        <th class="text-center" style="width: 20%;">รายละเอียดการชำระเงิน</th>
                ...
                <tbody class="g-font-size-14">
                    <tr v-for="(payment, index) in payments">
                        ...
                        <td>
                            {{payment.package.name}} - {{payment.package_duration}} : {{payment.package_amount}} บาท <span v-if="payment.package.package_type.name === 'Ads'"><button class="px-2 rounded border-0 py-0 cursor-pointer" type="button" data-toggle="modal" data-target="#modalAsset" @click="loadAssetlist(payment.ads.asset_id)" title="ดูรายละเอียดรายการประกาศ"><i class="fa fa-ellipsis-h g-font-size-20"></i></button></span><br>
                        ...
                        <td class="text-center"><button class="px-2 rounded border-0 py-1 cursor-pointer border-secondary" type="button" data-toggle="modal" data-target="#modalPaymentlines" @click="loadpaymentline(payment.id)"><i class="fa fa-ellipsis-h g-font-size-20"></i></button></td>
        ...
        <div class="modal fade" id="modalAsset" role="dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <table>
                        <tr>
                            <td style="width: 50%;"><img :src="asset_img" style="width: 100%;"></td>
                            <td valign="top" style="padding-left: 10px;">
                                <table class="table table-borderless" v-if="a_asset !== null">
                                    <tr>
                                        <td><h5>{{a_asset.name}}</h5></td>
                                    </tr>
                                    <tr>
                                        <td><strong>ประเภท :</strong> {{a_asset.asset_type.name}} <span v-if="a_asset.floor !== null"><strong>จำนวนชั้น :</strong> {{a_asset.floor}} ชั้น</span></td>
                                    </tr>
                                    <tr>
                                        <td><strong>ราคา :</strong> {{formatNumber(a_asset.price)}} ฿ <span v-if="a_asset.discount !== null"><strong>| ส่วนลด :</strong> {{formatNumber(a_asset.discount)}} ฿</span></td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <strong>รายละเอียด : </strong>
                                            <span v-if="a_asset.bedroom !== null">{{a_asset.bedroom}} ห้องนอน</span>
                                            <span v-if="a_asset.bathroom !== null"> , {{a_asset.bathroom}} ห้องน้ำ</span>
                                            <span v-if="a_asset.kitchenroom !== null"> , {{a_asset.kitchenroom}} ห้องครัว</span>
                                            <span v-if="a_asset.receptionroom !== null"> , {{a_asset.receptionroom}} ห้องรับแขก</span>
                                            <span v-if="a_asset.diningroom !== null"> , {{a_asset.diningroom}} ห้องทานอาหาร</span>
                                            <span v-if="a_asset.maidroom !== null"> , {{a_asset.maidroom}} ห้องแม่บ้าน</span>
                                            <span v-if="a_asset.parking !== null"> , {{a_asset.parking}} ที่จอดรถ</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <strong>ที่อยู่ : </strong>{{a_asset.address.address1}} ต.{{a_asset.address.subdistrict.name}} อ.{{a_asset.address.district.name}} จ.{{a_asset.address.province.name}}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td><strong>ระยะเวลาประกาศ :</strong> {{driffday(a_asset.startdate)}} วันที่ผ่านมา</td>
                                    </tr>
                                </table>
                            </td>
                        </th>
...
let packagepaymentlist = new Vue ({
    ...
    methods: {
        ...
        loadAssetlist: function (id) {
            this.loading.assetlist = true
            axios.get(apiurl + 'api-assets/get-asset?id=' + id)
            .then((response) => {
                // console.log(response)
                this.a_asset = response.data.asset
                this.asset_img = response.data.img
            })
            .catch(e => {
                console.log(e)
            })
            .finally(() => this.loading.assetlist = false)
        },
        ...
        upAds: function (id) {
            ...
            axios.post(apiurl + 'api-assets/up-asset-to-first', formData, {
                headers: {
