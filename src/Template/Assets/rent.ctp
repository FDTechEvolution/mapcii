...
<div class="g-bg-cover g-bg-pos-top-center g-bg-img-hero" style="background-image: url(<?= COVER_TITLE_IMAGE ?>);">
    <div class="g-pos-rel g-z-index-1">
        <div class="container g-pt-10">
            ...
            <h1 class="h2 mb-0 prompt-600 g-color-primary">ประกาศให้เช่า</h1>
            <div class="row">
                <div class="col-md-12">
...
<script>
    ...
    function getContent() {
        ...
        $.get(apiurl + 'api-assets/avaliable-asset?isnewproject=Y&' + search_url).done(function (res) {
            res = JSON.parse(res);
            console.log(res);
            var assets = res.list;
            $('#div_assetlist').empty();
    ...
    $(document).ready(function () {
        $('#page-load').show();
        $.get(apiurl + 'api-assets/avaliable-asset?province=1&isnewproject=Y').done(function (res) {
            res = JSON.parse(res);
            var assets = res.list;
            $('#div_assetlist').empty();
            var lat, lng;
