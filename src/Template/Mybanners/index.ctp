            ...
            <div class="col-md-12 mb-0">
                <?php
                    if($this->request->getSession()->read('Authen.User.islocked') == 'Y') {
                        ...
                        <h3 class="h3 mb-0 g-color-primary g-mr-20">รายการแบนเนอร์</h3>
                <?php
                    }
                ?>
            </div>
            <div class="col-md-4">
                <?=$this->Html->link('<i class="far fa-plus-square"></i> เพิ่มแบนเนอร์',['action'=>'add'],['class'=>"btn btn-outline-primary mb-0", 'style' => 'margin-bottom: -25px !important;', 'escape' => false])?>
            </div>
            <div class="col-md-8 text-right align-text-bottom pt-4 mb-2">
                <small>
                    <i class="fas fa-edit text-success"></i> = แก้ไขแบนเนอร์ , <i class="fas fa-redo-alt text-info"></i> = ต่ออายุแบนเนอร์ <u>(อายุ < 7 วัน)</u> , <i class="fas fa-level-up-alt text-primary"></i> = เลื่อนแบนเนอร์ขึ้นอันดับแรก <u>(วันละ 1 ครั้ง)</u> , <i class="fas fa-times text-danger"></i> = ปิดแบนเนอร์
                </small>
            </div>
